...
//! Server entry point for the Counter Example
//!
...
async fn main() -> std::io::Result<()> {
    ...
    use actix_web::*;
    use counter_example::components::App;
    use leptos::prelude::*;
    ...
    if std::env::var("LEPTOS_OUTPUT_NAME").is_err() {
        unsafe {
            std::env::set_var("LEPTOS_OUTPUT_NAME", "counter-example");
            std::env::set_var("LEPTOS_SITE_ROOT", "target/site");
            std::env::set_var("LEPTOS_SITE_PKG_DIR", "pkg");
            std::env::set_var("LEPTOS_SITE_ADDR", "127.0.0.1:3001");
        }
    }
    ...
    println!("🔢 Counter Example (SSR Mode)");
