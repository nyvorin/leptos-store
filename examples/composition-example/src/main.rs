...
//! Server entry point for the Composition Example
...
async fn main() -> std::io::Result<()> {
    ...
    use actix_web::*;
    use composition_example::components::App;
    use leptos::prelude::*;
    use leptos_actix::{LeptosRoutes, generate_route_list};

    if std::env::var("LEPTOS_OUTPUT_NAME").is_err() {
        unsafe {
            std::env::set_var("LEPTOS_OUTPUT_NAME", "composition-example");
            std::env::set_var("LEPTOS_SITE_ROOT", "target/site");
            std::env::set_var("LEPTOS_SITE_PKG_DIR", "pkg");
            std::env::set_var("LEPTOS_SITE_ADDR", "127.0.0.1:3030");
        }
    }
    ...
    println!("Composition Example (SSR Mode)");
