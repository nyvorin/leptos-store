...
//! Server entry point for the Persistence Example
...
async fn main() -> std::io::Result<()> {
    ...
    use persistence_example::components::App;

    if std::env::var("LEPTOS_OUTPUT_NAME").is_err() {
        unsafe {
            std::env::set_var("LEPTOS_OUTPUT_NAME", "persistence-example");
            std::env::set_var("LEPTOS_SITE_ROOT", "target/site");
            std::env::set_var("LEPTOS_SITE_PKG_DIR", "pkg");
            std::env::set_var("LEPTOS_SITE_ADDR", "127.0.0.1:3020");
        }
    }
    ...
    println!("Persistence Example (SSR Mode)");
