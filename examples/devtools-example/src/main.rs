...
//! Server entry point for the Devtools Example
...
async fn main() -> std::io::Result<()> {
    ...
    if std::env::var("LEPTOS_OUTPUT_NAME").is_err() {
        unsafe {
            std::env::set_var("LEPTOS_OUTPUT_NAME", "devtools-example");
            std::env::set_var("LEPTOS_SITE_ROOT", "target/site");
            std::env::set_var("LEPTOS_SITE_PKG_DIR", "pkg");
            std::env::set_var("LEPTOS_SITE_ADDR", "127.0.0.1:3050");
        }
    }
    ...
    println!("Devtools Example (SSR Mode)");
