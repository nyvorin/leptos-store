...
//! Server entry point for the Middleware Example
...
async fn main() -> std::io::Result<()> {
    ...
    use middleware_example::components::App;

    if std::env::var("LEPTOS_OUTPUT_NAME").is_err() {
        unsafe {
            std::env::set_var("LEPTOS_OUTPUT_NAME", "middleware-example");
            std::env::set_var("LEPTOS_SITE_ROOT", "target/site");
            std::env::set_var("LEPTOS_SITE_PKG_DIR", "pkg");
            std::env::set_var("LEPTOS_SITE_ADDR", "127.0.0.1:3010");
        }
    }
    ...
    println!("Middleware Example (SSR Mode)");
